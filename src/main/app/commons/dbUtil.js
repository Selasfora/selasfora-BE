...
exports.buildOptions = (filters, options) => {
  ...
  for (const key of _.keys(testOptions)) {
    ...
    if (parseInt(limit, 10) > 0) {
      testOptions[key].limit = parseInt(limit, 10);
    }
    ...
    if (parseInt(offset, 10) > 0) {
      testOptions[key].offset = parseInt(offset, 10);
    }
...
exports.fetchFilterCriteria = (query) => {
  Logger.info('dbUtil.fetchFilterCriteria - entry : filters received - ', query);
